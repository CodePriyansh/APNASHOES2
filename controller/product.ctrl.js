...
exports.selectCategory =(request,response,next)=>{
    Promise.all([Category.fetchAll(), Product.fatchAll()])
    .then(results=>{
        return response.render("../Views/Admin-Pages/product-list.ejs",{
...
exports.productByCategoryId =(request,response,next)=>{
    ...
    Promise.all([Category.fetchAll(), Product.fatchAllById(category_id)])
    .then(results=>{
        return response.render("../Views/Admin-Pages/product-list.ejs",{
            categoryList: results[0],
            productList: results[1]
